...
const filteredAndSortedOrders = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    list = list.filter(o => String(o.id).includes(q) || o.userId.includes(q))
  }
...
<template>
  <section class="max-w-5xl mx-auto p-6">
    <h1 class="text-2xl font-bold mb-4">Alle Bestellungen</h1>
    ...
    <table class="w-full border border-gray-200 text-left text-sm">
      ...
      <tr>
        ...
        <th class="px-4 py-2 border cursor-pointer" @click="sortBy('userId')">User ID</th>
      ...
      <tr v-for="o in filteredAndSortedOrders" :key="o.id" class="hover:bg-gray-50">
        ...
        <td class="px-4 py-2 border">{{ o.userId }}</td>
        ...
        <td class="px-4 py-2 border">
          <button class="text-blue-600 hover:underline mr-2" @click="viewDetails(o.id)">Details</button>
        </td>
      </tr>
      </tbody>
    </table>
